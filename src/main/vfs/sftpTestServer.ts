...
import { describeHostKey } from './hostKeys.js'
...
export interface TestServer {
  ...
  clientKey: string
  /**
   * The identity this server proves to clients, as the app would describe it.
   *
   * Exposed so a test can assert that the fingerprint the app displays is the
   * server's real one, rather than merely that some string came back.
   */
  ...
  /**
   * How many times a client has offered credentials.
   *
   * The evidence for the claim that a refused host key costs nothing: a client
   * that rejects the server's identity must abandon the handshake before this
   * ever moves off zero.
   */
  authAttempts: () => number
...
export interface TestServerOptions {
  ...
  password?: string
  /**
   * Bind a specific port rather than letting the OS choose.
   *
   * For one case: proving that a *changed* host key is caught. That needs a
   * different server answering at the address the first one had, which is only
   * expressible by reclaiming its port.
   */
  port?: number
}

export async function startSftpServer(root: string, options: TestServerOptions = {}): Promise<TestServer> {
  const { pair: hostPair, parsed: hostPublic } = generateUsableKeyPair('ed25519')
  const hostKey = hostPair.private
  ...
  const { pair: clientPair, parsed: clientPublic } = generateUsableKeyPair('rsa', { bits: 2048 })
  ...
  const server = new Server({ hostKeys: [hostKey] }, (client) => {
    ...
    client.on('authentication', (context) => {
      // Counted before the method is examined: what matters is that a client
      // got far enough to offer anything at all.
      if (context.method !== 'none') authAttempts += 1
  ...
  return {
    ...
    hostKey: describeHostKey(hostPublic.getPublicSSH()),
...
/**
 * A key pair ssh2 can read back, generated by ssh2.
 *
 * `generateKeyPairSync` emits a malformed pair roughly once in every 256 for
 * ed25519, and the cause is visible in the bytes: it writes the public point as
 * though it were a big integer, so a key whose first byte happens to be zero
 * loses it and the length field ends up one short. Both halves of that pair are
 * then unparseable — by ssh2 itself, and by anything else.
 *
 * That is an upstream defect and not one this project can fix, but it is also
 * not one to inherit: a suite that fails a few times in a hundred runs for a
 * reason unrelated to the code under test teaches everyone to re-run rather
 * than to read. Bad pairs are discarded rather than worked around, so what the
 * tests use is always a key the library agrees is well formed.
 */
function generateUsableKeyPair(
  type: 'ed25519' | 'rsa',
  options?: { bits: number }
): { pair: { private: string; public: string }; parsed: ssh2.ParsedKey } {
  for (let attempt = 0; attempt < 16; attempt++) {
    // Branched rather than passed through: ssh2 types the options argument per
    // algorithm, and only the RSA form takes a bit count.
    const pair =
      type === 'rsa'
    ...
    if (!(parsed instanceof Error) && !(utils.parseKey(pair.private) instanceof Error)) {
      return { pair, parsed }
    }
  }
  // Sixteen consecutive failures is not the one-in-256 case; something else is
  // wrong, and silently looping forever would be worse than saying so.
  throw new Error(`ssh2 could not generate a well-formed ${type} key pair`)
}
